 ...
 * 然后通过 {@link ChangeUrlInterceptor#setUrlParser(UrlParser)} 配置给框架, 即可替换 {@link DefaultUrlParser}
...
public class DefaultUrlParser implements UrlParser {
    ...
    private ChangeUrlInterceptor mChangeUrlInterceptor;

    @Override
    public void init(ChangeUrlInterceptor ChangeUrlInterceptor) {
        this.mChangeUrlInterceptor = ChangeUrlInterceptor;
        this.mDomainUrlParser = new DomainUrlParser();
        this.mDomainUrlParser.init(ChangeUrlInterceptor);
    }
    ...
    public HttpUrl parseUrl(HttpUrl domainUrl, HttpUrl url) {
        ...
        //如果是高级模式则使用高级解析器
        if (mChangeUrlInterceptor.isAdvancedModel()) {
            if (mAdvancedUrlParser == null) {
                synchronized (this) {
                    if (mAdvancedUrlParser == null) {
                        mAdvancedUrlParser = new AdvancedUrlParser();
                        mAdvancedUrlParser.init(mChangeUrlInterceptor);
                    }
                }
            }
